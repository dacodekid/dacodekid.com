...
import { renderSvg } from '../../plugins/mermaid/mermaid-render';
...
useIntersectionObserver(templateRef('root') as any, ([{ isIntersecting }]) => {
...
<template>
  <div ref="root">
    <template v-if="taskState && !taskState.pending">
      <pre v-if="taskState.fulfilled" data-mermaid class="flex justify-center" v-html="taskState.fulfilled.value.svg" />
      <template v-else>
        <div class="custom-block danger">
          <p class="custom-block-title">Unable to render the diagram</p>
    ...
    <div v-else class="flex justify-center p-8">Rendering the diagram...</div>
  </div>
</template>
